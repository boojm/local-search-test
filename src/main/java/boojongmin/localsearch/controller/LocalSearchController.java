...
public class LocalSearchController {
    ...
    @GetMapping({"/keyword/{keyword}" , "/keyword/{keyword}/page/{page}"})
    public LocationSearchModel locationSearch(@PathVariable("keyword") String keyword,
                                              @PathVariable(name="page", required = false) Optional<Integer> pageOpt) {
        return service.locationSearch(keyword, pageOpt);
    }
